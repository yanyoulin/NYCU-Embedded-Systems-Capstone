...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            ax1.plot([d[0] for d in accel_data], label='X')
            ax1.plot([d[1] for d in accel_data], label='Y')
            ax1.plot([d[2] for d in accel_data], label='Z')
            ax1.legend()

            # 更新陀螺儀數據圖表
            ax2.clear()
            ax2.set_title("Gyroscope Data")
            ax2.plot([d[0] for d in gyro_data], label='X')
            ax2.plot([d[1] for d in gyro_data], label='Y')
            ax2.plot([d[2] for d in gyro_data], label='Z')
            ax2.legend()

            # 更新磁力計數據圖表
            ax3.clear()
            ax3.set_title("Magnetometer Data")
            ax3.plot([d[0] for d in mag_data], label='X')
            ax3.plot([d[1] for d in mag_data], label='Y')
            ax3.plot([d[2] for d in mag_data], label='Z')
            ax3.legend()

            plt.pause(0.25)  # 暫停以更新圖表, 每秒約更新4次
